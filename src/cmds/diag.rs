    ...
    for level in Level::iter() {
        ...
        for card in cards {
            print!(
                ...
                card.item.bright_blue(),
                card.example.bright_yellow(),
                "ENTER".bright_white(),
                "SPACE".bright_white(),
                "Q".bright_white()
            );
                ...
                if let Event::Key(event) = event::read()? {
                    match event.code {
                        KeyCode::Enter => {
                            print!(
                                "\n{}\n",
                                TRAIN_SUCCESS[rng.random_range(0..TRAIN_SUCCESS.len())]
                                    .bright_green()
                            );
                        ...
                        KeyCode::Char(' ') => {
                            print!(
                                "\n{}\n",
                                TRAIN_FAILURE[rng.random_range(0..TRAIN_FAILURE.len())]
                                    .bright_red()
                            );
                            level_score.increase_total()?;
                        }
            ...
            // If quitting, break out of card loop as well (already broken by break)
            if exit_early {
                break;
            }
        }
        ...
        // disable raw mode (safety, might already be done)
        terminal::disable_raw_mode()?;

        // prevent next level – also stop if exiting
        if exit_early || (scores.last().map(|s| s.1).unwrap_or(1.0) < 0.75) {
            break;
        }
    }
